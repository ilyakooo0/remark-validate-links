  ...
  function mark(node) {
    ...
    if (node.children && node.children.length > 0) {
      for (child = 0; child < node.children.length; child++) {
        if (
          node.children[child].type === 'html' &&
          node.children[child + 1] &&
          node.children[child + 1].type === 'html'
        ) {
          ...
          if (
            matches &&
            matches.length === 3 &&
            node.children[child + 1].value === '</' + matches[1] + '>'
          ) {
    ...
    var id = props.name || anchorname || props.id || data.id
